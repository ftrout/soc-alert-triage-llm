...
class TriageEvaluator:
    ...
    def evaluate(
        ...
        result.priority_within_one = sum(1 for e in priority_errors if e <= 1) / len(priority_errors)
        result.priority_correlation = self._spearman_correlation(
            [float(p) for p in pred_priorities],
            [float(g) for g in true_priorities]
        )
        ...
        if (result.escalation_precision + result.escalation_recall) > 0:
            result.escalation_f1 = (
                2 * result.escalation_precision * result.escalation_recall /
                (result.escalation_precision + result.escalation_recall)
            )
        
        # Per-category metrics
        categories = set(p.get("category") for p in self.predictions if p.get("category"))
    ...
    def generate_report(
        ...
        if result.category_metrics:
            ...
            for category, metrics in sorted(result.category_metrics.items()):
                report.append(f"  {category:25} Accuracy: {metrics['accuracy']:.2%} (n={metrics['support']})")
