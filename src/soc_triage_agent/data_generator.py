...
class AlertCategory(Enum):
    ...
    def mitre_tactics(self) -> List[str]:
        ...
        mapping = {
            AlertCategory.MALWARE: ["TA0002", "TA0003", "TA0005"],  # Execution, Persistence, Defense Evasion
...
class AlertTemplates:
    ...
    MALWARE_TEMPLATES = {
        ...
        "sources": [
            ...
            "Carbon Black", "SentinelOne", "Symantec Endpoint Protection",
            "Cisco Secure Endpoint", "Trend Micro Apex One", "Sophos Intercept X"
        ],
        "malware_families": [
            "Emotet", "TrickBot", "QakBot", "Cobalt Strike", "Ryuk",
            "Conti", "LockBit", "BlackCat", "REvil", "Dridex"
        ],
    }
    ...
    PHISHING_TEMPLATES = {
        ...
        "sources": [
            "Proofpoint", "Microsoft Defender for Office 365", "Mimecast",
            "Abnormal Security", "Barracuda Email Security", "Cisco Secure Email"
        ],
        "subject_lines": [
    ...
    BRUTE_FORCE_TEMPLATES = {
        ...
        "sources": [
            "Azure AD Identity Protection", "Okta", "CrowdStrike Falcon Identity",
            "Duo Security", "SailPoint", "Ping Identity", "Microsoft Sentinel"
        ],
    }
    ...
    DATA_EXFILTRATION_TEMPLATES = {
        ...
        "sources": [
            ...
            "Varonis", "Digital Guardian", "Forcepoint DLP", "Symantec DLP"
        ],
    ...
    PRIVILEGE_ESCALATION_TEMPLATES = {
        ...
        "sources": [
            "CrowdStrike Falcon", "Microsoft Defender for Identity",
            "Tenable.ad", "Varonis", "SentinelOne", "Elastic Security"
        ],
        "techniques": [
            "DLL hijacking", "unquoted service path", "token impersonation",
            "kernel exploit", "misconfigured SUID", "capability abuse"
        ],
    }
    ...
    LATERAL_MOVEMENT_TEMPLATES = {
        ...
        "sources": [
            "Microsoft Defender for Identity", "CrowdStrike Falcon",
            "Vectra AI", "ExtraHop", "Darktrace", "Microsoft Sentinel"
        ],
    }
    ...
    C2_TEMPLATES = {
        ...
        "sources": [
            "Palo Alto Networks", "Cisco Umbrella", "Zscaler",
            "CrowdStrike Falcon", "Darktrace", "Vectra AI", "ExtraHop"
        ],
        "c2_frameworks": [
            "Cobalt Strike", "Metasploit", "Empire", "Covenant",
            "Sliver", "Brute Ratel", "Havoc", "Mythic"
        ],
    }
    ...
    INSIDER_THREAT_TEMPLATES = {
        ...
        "sources": [
            "Microsoft Insider Risk Management", "Securonix", "Exabeam",
            "Varonis", "ObserveIT", "DTEX", "Code42 Incydr"
        ],
    }
    ...
    POLICY_VIOLATION_TEMPLATES = {
        ...
        "sources": [
            "Microsoft Intune", "Tanium", "Qualys", "ServiceNow",
            "BigFix", "Jamf", "VMware Workspace ONE"
        ],
    ...
    VULNERABILITY_TEMPLATES = {
        ...
        "sources": [
            "Tenable", "Qualys", "Rapid7", "Microsoft Defender",
            "AWS GuardDuty", "Palo Alto Cortex XDR", "Snyk"
        ],
        "attack_types": [
            "SQL Injection", "XSS", "CSRF", "Command Injection",
            "LDAP Injection", "XML Injection", "Buffer Overflow"
        ],
    }
    ...
    RECONNAISSANCE_TEMPLATES = {
        ...
        "sources": [
            "Palo Alto Networks", "Cisco Firepower", "Suricata",
            "Zeek", "AWS GuardDuty", "Azure Sentinel", "Darktrace"
        ],
    }
    ...
    DOS_TEMPLATES = {
        ...
        "sources": [
            ...
            "Arbor Networks", "Radware", "Imperva"
        ],
    }
...
class SecurityAlertGenerator:
    ...
    def _generate_user_context(self) -> UserContext:
        ...
        departments = ["Engineering", "Finance", "HR", "Sales", "IT", "Legal", "Marketing", "Operations", "Executive", "Research"]
        roles = ["Analyst", "Manager", "Director", "VP", "Developer", "Administrator", "Specialist", "Coordinator", "Intern", "Consultant"]
        locations = ["New York", "San Francisco", "London", "Tokyo", "Singapore", "Austin", "Seattle", "Chicago", "Remote", "Berlin"]
    ...
    def _generate_asset_context(self) -> AssetContext:
        ...
        asset_types = ["workstation", "server", "laptop", "domain_controller", "database", "web_server", "file_server", "mail_server"]
        os_options = ["Windows 11", "Windows 10", "Windows Server 2022", "Windows Server 2019", "Ubuntu 22.04", "RHEL 8", "macOS Ventura", "CentOS 7"]
        business_units = ["Corporate", "Engineering", "Finance", "HR", "IT", "Sales", "Marketing", "Research"]
        
        asset_type = random.choice(asset_types)
        prefix = {"workstation": "WS", "server": "SRV", "laptop": "LAP", "domain_controller": "DC", "database": "DB"}.get(asset_type, "HOST")
        ...
        return AssetContext(
            ...
            criticality=random.choices(["critical", "high", "medium", "low"], weights=[0.1, 0.25, 0.45, 0.2])[0],
            data_classification=random.choices(["public", "internal", "confidential", "restricted"], weights=[0.2, 0.4, 0.3, 0.1])[0],
            patch_status=random.choices(["current", "behind", "critical_missing"], weights=[0.6, 0.3, 0.1])[0],
    ...
    def _generate_network_context(self, external_threat: bool = False) -> NetworkContext:
        ...
        return NetworkContext(
            ...
            destination_ip=self._generate_ip(internal=random.random() > 0.5) if random.random() > 0.3 else None,
            source_zone="external" if external_threat else random.choice(["internal", "vpn", "guest"]),
    ...
    def _generate_environment_context(self) -> EnvironmentContext:
        ...
        return EnvironmentContext(
            ...
            active_incidents=random.choices([0, 1, 2, 3, 4, 5], weights=[0.5, 0.25, 0.12, 0.08, 0.03, 0.02])[0],
            threat_level=random.choices(["normal", "elevated", "high", "critical"], weights=[0.7, 0.2, 0.08, 0.02])[0],
            recent_deployments=random.sample(["web-app-v2.1", "database-patch", "security-update", "new-feature"], k=random.randint(0, 2)),
        )
    
    def _generate_raw_log(self, category: AlertCategory, indicators: Dict[str, Any], timestamp: str) -> str:
        ...
        try:
            ...
            return f"{timestamp} ALERT: category={category.value} indicators={json.dumps(indicators)}"
    ...
    def _generate_indicators(self, category: AlertCategory) -> Dict[str, Any]:
        ...
        if category == AlertCategory.MALWARE:
            return {
                ...
                "file_name": random.choice(["svchost.exe", "update.exe", "installer.msi", "document.pdf.exe", "chrome_update.exe", "winlogon.exe"]),
                "file_path": random.choice(["C:\\Users\\Public\\", "C:\\Windows\\Temp\\", "C:\\ProgramData\\", "/tmp/", "/var/tmp/"]),
                "process_id": random.randint(1000, 65535),
                "parent_process": random.choice(["explorer.exe", "cmd.exe", "powershell.exe", "outlook.exe", "winword.exe"]),
                "detection_method": random.choice(["signature", "behavioral", "heuristic", "machine_learning", "sandbox"]),
                "malware_family": random.choice(AlertTemplates.MALWARE_TEMPLATES["malware_families"]),
            ...
            return {
                ...
                "sender_display_name": random.choice(["IT Support", "HR Department", "CEO Office", "Microsoft Support", "Help Desk"]),
                "subject_line": random.choice(AlertTemplates.PHISHING_TEMPLATES["subject_lines"]).format(
                ...
                "malicious_urls": [f"https://{self._generate_domain(suspicious=True)}/login" for _ in range(random.randint(1, 3))],
                "attachment_count": random.randint(0, 2),
                "attachment_types": random.sample([".docx", ".xlsx", ".pdf", ".html", ".zip"], k=random.randint(0, 2)),
            ...
            return {
                ...
                "target_account_list": [f"user{i}@company.com" for i in range(random.randint(1, 5))],
            ...
            return {
                ...
                "destination_service": random.choice(["Dropbox", "Google Drive", "OneDrive", "WeTransfer", "Custom", "Unknown"]),
                "file_types": random.sample(["pdf", "xlsx", "docx", "zip", "sql", "csv", "pst", "rar"], k=random.randint(1, 4)),
            ...
            return {
                "original_privilege": random.choice(["user", "guest", "service_account", "standard"]),
                "target_privilege": random.choice(["local_admin", "domain_admin", "system", "root", "enterprise_admin"]),
                "technique": random.choice(AlertTemplates.PRIVILEGE_ESCALATION_TEMPLATES["techniques"]),
                "mitre_technique": random.choice(["T1548", "T1134", "T1068", "T1078", "T1055", "T1574"]),
                "success": random.choice([True, False]),
                "tool_used": random.choice(["Mimikatz", "Rubeus", "PowerUp", "WinPEAS", "Custom", "Unknown"]),
            ...
            return {
                ...
                "destination_hosts": [f"SRV-{random.randint(10, 99)}" for _ in range(random.randint(1, 10))],
                "destination_count": random.randint(1, 10),
                "protocol": random.choice(["SMB", "WMI", "RDP", "WinRM", "SSH", "PSRemoting"]),
                "credentials_type": random.choice(["pass_the_hash", "pass_the_ticket", "valid_credentials", "stolen_token", "service_account"]),
                "tool_used": random.choice(["PsExec", "WMIExec", "Invoke-Command", "CrackMapExec", "Impacket", "Native"]),
                "mitre_technique": random.choice(["T1021.001", "T1021.002", "T1021.003", "T1021.004", "T1021.006"]),
            }
            ...
            return {
                ...
                "c2_framework": random.choice(AlertTemplates.C2_TEMPLATES["c2_frameworks"]) if random.random() > 0.5 else "Unknown",
            ...
            return {
                ...
                "behavior_anomaly": random.choice(["access_time", "data_volume", "resource_access", "geographic", "device_change", "pattern_deviation"]),
                "employment_status": random.choice(["active", "notice_period", "terminated", "contractor"]),
                "historical_violations": random.randint(0, 5),
                "data_accessed_classification": random.choice(["internal", "confidential", "restricted"]),
            ...
            return {
                "policy_name": random.choice(["Software Installation Policy", "Remote Access Policy", "Data Classification Policy", "Encryption Policy", "Acceptable Use Policy"]),
                "violation_type": random.choice(["unauthorized_app", "config_drift", "missing_encryption", "unapproved_access", "data_handling"]),
                "compliance_framework": random.choice(AlertTemplates.POLICY_VIOLATION_TEMPLATES["frameworks"]),
                "remediation_required": random.choice([True, False]),
                "repeat_violation": random.choice([True, False, False, False]),  # 25% repeat
                "software_name": random.choice(["Dropbox", "TeamViewer", "AnyDesk", "Slack", "WhatsApp", "Unknown"]) if random.random() > 0.5 else None,
            }
            ...
            return {
                ...
                "attack_type": random.choice(AlertTemplates.VULNERABILITY_TEMPLATES["attack_types"]),
                ...
                "affected_service": random.choice(["Apache", "Nginx", "IIS", "Tomcat", "Exchange", "SharePoint", "Custom"]),
            }
        
        elif category == AlertCategory.RECONNAISSANCE:
            return {
                "scan_type": random.choice(["port_scan", "vulnerability_scan", "network_sweep", "service_enumeration", "os_fingerprinting"]),
                ...
                "scan_tool": random.choice(["Nmap", "Masscan", "Nessus", "OpenVAS", "Custom", "Unknown"]),
                "stealth_techniques": random.choice([True, False]),
            }
        
        elif category == AlertCategory.DENIAL_OF_SERVICE:
            return {
                "attack_type": random.choice(["volumetric", "protocol", "application_layer", "amplification"]),
                "attack_vector": random.choice(["SYN Flood", "UDP Flood", "HTTP Flood", "DNS Amplification", "NTP Amplification", "Slowloris"]),
    ...
    def _determine_triage(
        ...
        if category == AlertCategory.MALWARE:
            if indicators.get("detection_method") == "signature":
                key_factors.append(f"Known malware family detected: {indicators.get('malware_family', 'Unknown')}")
                ...
                key_factors.append("Behavioral/heuristic detection may indicate new variant or false positive")
            ...
            if indicators.get("successful_auth"):
                ...
                key_factors.append(f"High volume attack: {indicators.get('failed_attempts')} attempts detected")
            ...
            key_factors.append(f"Movement to {indicators.get('destination_count', 1)} systems via {indicators.get('protocol', 'unknown')}")
            ...
            if indicators.get("threat_intel_match"):
                ...
                ioc_extraction.append(f"C2 Domain: {indicators.get('destination_domain', 'Unknown')}")
        ...
        if environment_context.threat_level in ["high", "critical"]:
            priority = max(1, priority - 1)
            key_factors.append(f"Elevated organization threat level: {environment_context.threat_level}")
    ...
    def generate_alert(
        ...
        network_context = self._generate_network_context(
            external_threat=category in [AlertCategory.RECONNAISSANCE, AlertCategory.DENIAL_OF_SERVICE, AlertCategory.BRUTE_FORCE]
        )
        ...
        try:
            title = title_template.format(**{
                **indicators,
                ...
                "volume": indicators.get("volume_mb", indicators.get("traffic_volume_gbps", 100)),
                "sender_domain": indicators.get("sender_email", "unknown@unknown.com").split("@")[-1],
                "file_type": random.choice(indicators.get("attachment_types", [".pdf"])) if indicators.get("attachment_types") else ".pdf",
                ...
                "resource": random.choice(["Finance Share", "HR Database", "Executive Folder", "Source Code"]),
            ...
            title = title_template.split("{")[0].strip() if "{" in title_template else title_template
        ...
        triage = self._determine_triage(
            ...
            user_context, asset_context, environment_context
        )
...
def main():
    ...
    parser.add_argument("--format", choices=["chat", "instruction", "completion", "sharegpt", "huggingface"], 
                        default="chat", help="Output format")
